/**
 * edVenture — Contact page (drop-in for a Next.js App Router + Tailwind CSS app)
 *
 * HOW TO USE
 * 1. Copy this file to  app/contact/page.tsx  (or replace your existing contact page).
 * 2. Copy the two illustrations to  public/images/hero-illustration.png  and
 *    public/images/mailbox-illustration.png  (prompts to regenerate them are in
 *    design-prompt.md — any AI image tool will do).
 * 3. This file uses only Tailwind utilities (arbitrary values) + inline SVGs.
 *    It references one class from your existing design system: `font-heading`.
 *    If you don't have it, add to tailwind.config:
 *      theme.extend.fontFamily.heading = ["Poppins", "ui-sans-serif", "system-ui"]
 * 4. If your root layout already renders a fixed site header, delete <SiteHeader />
 *    below (and keep your existing pt-[76px] on <main>).
 */
...
const LogoMark = () => (
  <svg width="42" height="42" viewBox="0 0 48 48" aria-hidden="true">
    <defs>
      <linearGradient id="edv-logo-g" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#7C3AED" />
        <stop offset="1" stopColor="#2563EB" />
      </linearGradient>
    </defs>
    <path d="M24 3 42 13.5v21L24 45 6 34.5v-21Z" fill="url(#edv-logo-g)" />
    <path d="M30.5 19.5a8 8 0 1 0 0 9" fill="none" stroke="#fff" strokeWidth="4.5" strokeLinecap="round" />
  </svg>
);

/* ------------------------------------------------------------------ */
/* Header (delete if your layout already renders a global header)      */
/* ------------------------------------------------------------------ */

function SiteHeader() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#EEEFF6] bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-[76px] max-w-[1200px] items-center justify-between px-5">
        <a href="/" className="flex items-center gap-2.5">
          <LogoMark />
          <span className="leading-tight">
            <span className="block font-heading text-[19px] font-bold tracking-tight text-[#101331]">
              ed<span className="text-[#4B2FE2]">Venture</span>
            </span>
            <span className="block text-[9px] font-semibold uppercase tracking-[0.14em] text-[#7A8194]">
              Immersive eLearning Solutions
            </span>
          </span>
        </a>
        <nav className="hidden items-center gap-9 text-[15px] font-medium md:flex">
          {NAV_LINKS.map((link) => (
            <a
              ...
              className={
                link === "Contact"
                  ? "font-semibold text-[#101331]"
                  : "text-[#3B4152] transition hover:text-[#4B2FE2]"
              }
            >
              {link}
            </a>
...
export default function ContactPage() {
  return (
    <div className="min-h-screen bg-white text-[#101331]">
      <SiteHeader />
